...
Usage:
    python lot_sizes.py                  # scan cache
    python lot_sizes.py --fetch          # top up the cache to today first
"""
...
def wheel_impact(events: pd.DataFrame, d: pd.DataFrame, names: list[str],
    ...
    """
    per_name = p["setup"]["initial_capital"] / p["setup"]["n_names"]
    otm = p["put"]["otm_pct"]
    one_lot_mult = p["sizing"]["one_lot_max_multiple"]
    rows = []
...
def main() -> None:
    ...
    names = params["universe"]["frozen_names"] + ["TMPV", "SAMMAANCAP"]
